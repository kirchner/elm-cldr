    ...
    if
        (floor (Plural.absoluteValue '.' count) % 10 == 0)
            || (((floor (Plural.absoluteValue '.' count) % 100 < 11) && (floor (Plural.absoluteValue '.' count) % 100 > 19))
                    || ((Plural.fractionDigitCount '.' WithTrailingZeros count == 2)
                            && ((Plural.fractionDigits '.' WithTrailingZeros count % 100 < 11) && (Plural.fractionDigits '.' WithTrailingZeros count % 100 > 19))
                       )
               )
    then
        Zero
