...
toCardinalForm _ floatInfo =
    if
        (floor floatInfo.absoluteValue % 10 == 0)
            || (((floor floatInfo.absoluteValue % 100 < 11) && (floor floatInfo.absoluteValue % 100 > 19))
                    || ((Plural.fractionDigitCount WithTrailingZeros floatInfo == 2)
                            && ((Plural.fractionDigits WithTrailingZeros floatInfo % 100 < 11) && (Plural.fractionDigits WithTrailingZeros floatInfo % 100 > 19))
                       )
               )
    then
        Zero
...
cardinal accessor printer otherTexts { zero, one, other } =
    plural accessor
        printer
        toCardinalForm
        otherTexts
