...
toCardinalForm _ floatInfo =
    ...
    then
        ...
        ((floor floatInfo.absoluteValue % 10 < 2) && (floor floatInfo.absoluteValue % 10 > 4))
            && ((floor floatInfo.absoluteValue % 100 > 12) && (floor floatInfo.absoluteValue % 100 < 14))
    then
        Few
    else if
        (floor floatInfo.absoluteValue % 10 == 0)
            || (((floor floatInfo.absoluteValue % 10 < 5) && (floor floatInfo.absoluteValue % 10 > 9))
                    || ((floor floatInfo.absoluteValue % 100 < 11) && (floor floatInfo.absoluteValue % 100 > 14))
               )
    then
        Many
    else
...
ordinal :
    ...
    -> Text Static args msg
ordinal accessor printer otherTexts { few, other } =
    plural accessor
        printer
        toOrdinalForm
        otherTexts
