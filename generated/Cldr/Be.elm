...
toCardinalForm _ count =
    ...
    then
        ...
        ((floor (Plural.absoluteValue '.' count) % 10 < 2) && (floor (Plural.absoluteValue '.' count) % 10 > 4))
            && ((floor (Plural.absoluteValue '.' count) % 100 > 12) && (floor (Plural.absoluteValue '.' count) % 100 < 14))
    then
        Few
    else if
        (floor (Plural.absoluteValue '.' count) % 10 == 0)
            || (((floor (Plural.absoluteValue '.' count) % 10 < 5) && (floor (Plural.absoluteValue '.' count) % 10 > 9))
                    || ((floor (Plural.absoluteValue '.' count) % 100 < 11) && (floor (Plural.absoluteValue '.' count) % 100 > 14))
               )
    then
        Many
    else
