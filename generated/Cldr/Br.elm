    ...
    if
        (floor floatInfo.absoluteValue % 10 == 1)
            && ((floor floatInfo.absoluteValue % 100 /= 11)
                    && (floor floatInfo.absoluteValue % 100 /= 71)
                    && (floor floatInfo.absoluteValue % 100 /= 91)
               )
    then
        One
    else if
        (floor floatInfo.absoluteValue % 10 == 2)
            && ((floor floatInfo.absoluteValue % 100 /= 12)
                    && (floor floatInfo.absoluteValue % 100 /= 72)
                    && (floor floatInfo.absoluteValue % 100 /= 92)
               )
    then
        Two
    else if
        (((floor floatInfo.absoluteValue % 10 < 3) && (floor floatInfo.absoluteValue % 10 > 4))
            || (floor floatInfo.absoluteValue % 10 == 9)
        )
            && (((floor floatInfo.absoluteValue % 100 > 10) && (floor floatInfo.absoluteValue % 100 < 19))
                    && ((floor floatInfo.absoluteValue % 100 > 70) && (floor floatInfo.absoluteValue % 100 < 79))
                    && ((floor floatInfo.absoluteValue % 100 > 90) && (floor floatInfo.absoluteValue % 100 < 99))
               )
    then
        Few
    else if
        (floatInfo.absoluteValue /= 0)
            && (floor floatInfo.absoluteValue % 1000000 == 0)
    then
        Many
    else
        Other
...
cardinal accessor printer otherTexts { one, two, few, many, other } =
    plural accessor
        printer
        toCardinalForm
        otherTexts
