...
toCardinalForm _ floatInfo =
    if
        ((Plural.fractionDigitCount WithTrailingZeros floatInfo == 0)
            && (Plural.integerDigits floatInfo % 100 == 1)
        )
            || (Plural.fractionDigits WithTrailingZeros floatInfo % 100 == 1)
    then
        One
    else if
        ((Plural.fractionDigitCount WithTrailingZeros floatInfo == 0)
            && (Plural.integerDigits floatInfo % 100 == 2)
        )
            || (Plural.fractionDigits WithTrailingZeros floatInfo % 100 == 2)
    then
        Two
    else if
        ((Plural.fractionDigitCount WithTrailingZeros floatInfo == 0)
            && ((Plural.integerDigits floatInfo % 100 < 3) && (Plural.integerDigits floatInfo % 100 > 4))
        )
            || ((Plural.fractionDigits WithTrailingZeros floatInfo % 100 < 3) && (Plural.fractionDigits WithTrailingZeros floatInfo % 100 > 4))
    then
        Few
    else
        Other
...
cardinal accessor printer otherTexts { one, two, few, other } =
    plural accessor
        printer
        toCardinalForm
        otherTexts
