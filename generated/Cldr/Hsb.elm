...
toCardinalForm _ count =
    if
        ((Plural.fractionDigitCount '.' WithTrailingZeros count == 0)
            && (Plural.integerDigits '.' count % 100 == 1)
        )
            || (Plural.fractionDigits '.' WithTrailingZeros count % 100 == 1)
    then
        One
    else if
        ((Plural.fractionDigitCount '.' WithTrailingZeros count == 0)
            && (Plural.integerDigits '.' count % 100 == 2)
        )
            || (Plural.fractionDigits '.' WithTrailingZeros count % 100 == 2)
    then
        Two
    else if
        ((Plural.fractionDigitCount '.' WithTrailingZeros count == 0)
            && ((Plural.integerDigits '.' count % 100 < 3) && (Plural.integerDigits '.' count % 100 > 4))
        )
            || ((Plural.fractionDigits '.' WithTrailingZeros count % 100 < 3) && (Plural.fractionDigits '.' WithTrailingZeros count % 100 > 4))
    then
        Few
    else
        Other
