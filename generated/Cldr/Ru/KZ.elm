...
toCardinalForm _ count =
    ...
    then
        ...
        ((Plural.fractionDigitCount '.' WithTrailingZeros count == 0)
            && (Plural.integerDigits '.' count % 10 == 0)
        )
            || (((Plural.fractionDigitCount '.' WithTrailingZeros count == 0)
                    && ((Plural.integerDigits '.' count % 10 < 5) && (Plural.integerDigits '.' count % 10 > 9))
                )
                    || ((Plural.fractionDigitCount '.' WithTrailingZeros count == 0)
                            && ((Plural.integerDigits '.' count % 100 < 11) && (Plural.integerDigits '.' count % 100 > 14))
                       )
               )
    then
        Many
